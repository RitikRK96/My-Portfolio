...
const BOOK_STATUS: Record<string, { label: string; cls: string; dot: string }> =
  {
    draft: {
      label: "Draft",
      cls: "text-gray-400 bg-white/[0.05] border-white/10",
      dot: "bg-gray-500",
    },
    in_progress: {
      label: "In Progress",
...
const BookDetail = () => {
      ...
      try {
        ...
        if (data.outline) {
          ...
          localStorage.setItem(
            `bw-book-${bookId}-outline`,
            JSON.stringify(data.outline),
          );
        } else {
          try {
            const local = JSON.parse(
              localStorage.getItem(`bw-book-${bookId}-outline`) || "[]",
            );
            setOutline(local);
          } catch {
            setOutline([]);
  ...
  useEffect(() => {
    if (!bookId) return;
    localStorage.setItem(`bw-book-${bookId}-outline`, JSON.stringify(outline));

    if (!outlineLoadedRef.current) return;
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.error("Failed to auto-save outline in detail page:", err);
      }
    }, 1500);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const reordered = arrayMove(activeChapters, oldIndex, newIndex).map(
      (c, i) => ({ ...c, order: i }),
    );
        ...
        prev
          ? {
              ...
              chapters: prev.chapters.map((c) =>
                c.id === chapterToDelete ? { ...c, isDeleted: true } : c,
              ),
            }
          : prev,
      );
  ...
  const confirmHardDeleteChapter = async () => {
    ...
    try {
      const token = await auth.currentUser?.getIdToken();
      await fetch(
        `${API_URL}/${bookId}/chapters/${chapterToHardDelete}?hard=true`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      setBook((prev) =>
        prev
          ? {
              ...prev,
              chapters: prev.chapters.map((c) =>
                c.id === chapterId ? { ...c, isDeleted: false } : c,
              ),
            }
          : prev,
      );
  ...
  const totalWords = activeChapters.reduce(
            ...
            {isEditingBook ? (
              <div className="flex-1 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white/[0.04] border border-white/[0.1] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-orange-500/50"
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white/[0.04] border border-white/[0.1] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-orange-500/50 appearance-none"
                    >
                      <option value="draft" className="bg-[#0a0a12]">
                        Draft
                      </option>
                      <option value="in_progress" className="bg-[#0a0a12]">
                        In Progress
                      </option>
                      <option value="published" className="bg-[#0a0a12]">
                        Published
                      </option>
                      <option value="archived" className="bg-[#0a0a12]">
                        Archived
                      </option>
                    </select>
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-white/[0.04] border border-white/[0.1] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-orange-500/50 resize-none"
                  />
                </div>
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    disabled={savingBook}
                    className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-400 text-black rounded-lg text-sm font-semibold transition-colors disabled:opacity-50"
                  >
                    {savingBook ? (
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-white/[0.05] transition-all"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-3 mb-2">
                  ...
                  <span
                    className={clsx(
                      "flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold rounded-full border",
                      bs.cls,
                    )}
                  >
                    <span
                      className={clsx("w-1.5 h-1.5 rounded-full", bs.dot)}
                    />
                    {bs.label}
                  </span>
                </div>
                {book.genre && (
                  <p className="text-sm text-orange-400/70 font-medium mb-3">
                    {book.genre}
                  </p>
                )}
                {book.description && (
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs text-gray-600">
                  ...
                  <span className="flex items-center gap-1.5">
                    <Clock size={12} />~{readTime}m read
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Hash size={12} />
                    {activeChapters.length} chapter
                    {activeChapters.length !== 1 ? "s" : ""}
                  </span>
                  {book.updatedAt && (
                    <span className="flex items-center gap-1.5">
                      <Calendar size={12} />
        ...
        <div className="bg-[#07070e] border border-white/[0.07] rounded-2xl overflow-hidden mb-20">
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] flex-wrap gap-3">
            <div className="flex items-center gap-6 flex-wrap">
              ...
              >
                <FileText
                  size={15}
                  className={activeTab === "chapters" ? "text-orange-400" : ""}
                />
                Chapters
                <span
                  className={clsx(
              ...
              >
                <ScrollText
                  size={15}
                  className={activeTab === "outline" ? "text-orange-400" : ""}
                />
                Outline & Notes
              </button>
              ...
              <button
                ...
                className={clsx(
                  ...
                  activeTab === "recycle_bin"
                    ? "text-red-400"
                    : "text-gray-500 hover:text-red-400/70",
                )}
              >
                <Trash2 size={15} />
                Recycle Bin
                  ...
                  <div className="flex flex-col">
                    {activeChapters.map((ch, i) => (
                      <SortableChapterRow
                        ...
                        onClick={() =>
                          navigate(
                            `/admin/book-writer?bookId=${bookId}&chapterId=${ch.id}`,
                          )
                        }
                        onDelete={(e, id) => {
                          e.stopPropagation();
          ...
          {activeTab === "outline" && (
            <div className="max-w-3xl mx-auto p-6">
              <div className="flex flex-col bg-[#07070e] border border-white/[0.06] rounded-2xl p-6 min-h-[400px] overflow-hidden">
                <div className="flex items-center justify-between mb-6 flex-shrink-0">
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                    <ScrollText size={16} className="text-orange-400" />
                    Tasks & Outline Goals
                  </h3>
                  <button
                    type="button"
                ...
                <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 scrollbar-thin scrollbar-thumb-white/10">
                  {outline.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-start gap-3 group border border-white/[0.04] bg-white/[0.01] hover:bg-white/[0.03] rounded-xl p-3.5 transition-colors"
                    >
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="text-gray-500 hover:text-orange-400 mt-0.5 flex-shrink-0 cursor-pointer"
                      >
                        {item.completed ? (
                      ...
                      <input
                        ...
                        className={clsx(
                          "flex-1 bg-transparent text-sm text-white outline-none placeholder:text-gray-700 border-b border-transparent focus:border-orange-500/20 pb-0.5",
                          item.completed &&
                            "line-through text-gray-500 font-medium",
                        )}
                        placeholder="Write plot point..."
                      />
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1 text-gray-750 hover:text-red-400 transition-all flex-shrink-0 cursor-pointer"
                      >
                        <X size={14} />
                      </button>
                    </div>
                  ...
                  {outline.length === 0 && (
                    <div className="text-center py-20 text-gray-700">
                      <p className="text-xs">No goals added yet</p>
                      <p className="text-[10px] opacity-75 mt-1">
                        Add items to keep track of your plot milestones.
                      </p>
                    </div>
                  )}
                </div>
            ...
            (deletedChapters.length === 0 ? (
              ...
              <div className="divide-y divide-white/[0.04]">
                {deletedChapters.map((ch) => (
                  <div
                    key={ch.id}
                    className="flex items-center justify-between px-6 py-4 bg-[#07070e] hover:bg-white/[0.02] transition-colors"
                  >
                    <div>
                      ...
                      <p className="text-[11px] text-gray-600 mt-0.5">
                        {fmtWords(countWords(ch.content ?? ""))} words
                      </p>
                    </div>
